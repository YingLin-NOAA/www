...
image_name = "loop-images/";
//image_name = "/sod/mecb/goes_validation/sst-img/";
image_type = "png";
var_type = "pcpurma_24h";
first_image = 1;
last_image = 20;
...
//===> Preload the first image (while page is downloading)
   ...
   theImages[0].src = image_name + first_image + "_" + var_type +  "." + image_type;
   imageNum[0] = true;
...
function launch()
{
   for (var i = first_image; i <= last_image; i++)
   {
      ...
      theImages[i-first_image].src = image_name + i + "_" + var_type + "." + image_type;
  ...
  // set image for available files
  theFiles = new Image();
  theFiles.src = image_name + "." + image_type;
  document.files.src = theFiles.src;
}
 
//===> Check selection status of image in animation loop
function checkImage(pstatus,i)
{
   if (pstatus == true)
      imageNum[i] = false;
